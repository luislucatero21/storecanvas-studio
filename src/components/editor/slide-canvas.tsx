...
function resolvedRectFor(
  ...
  const rect = rectFor(id, slide, defaults);
  if (!rect) return undefined;
  const transform = getElementTransform(slide, device, orientation, id, locale);
  return transform ? { ...rect, ...transform } : rect;
}
...
function SlideElements({
  ...
  const screenshot = resolveAssetPath(slide.assetRef, locale, assets, slide.screenshot);
  const screenshotSecondary = resolveAssetPath(
  ...
  function renderTextElement(textElement: TextElement, index: number) {
    ...
    if (isHidden(elementId)) return null;
...
function Movable({
  ...
  return (
    <Rnd
      ...
      className={`${selected ? "rnd-editable rnd-selected" : "rnd-editable"}${locked ? " rnd-locked" : ""}`}
    >
      {rotated}
      {!locked && (
        <>
